  ...
  if('serviceWorker' in navigator){
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('./sw.js')
        ...
        .catch((error) => {
          console.error("Error registering ServiceWorkers", error);
        });
    });
  }
...
async function getRecipes() {
  ...
  let initial_recipes = JSON.parse(localStorage.getItem('recipes'));
  if(initial_recipes){
  ...
  return new Promise(async (resolve, reject) => {
    for(let elem of RECIPE_URLS){
      try{
        let url = await fetch(elem);
